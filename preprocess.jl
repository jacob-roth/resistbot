...
function display_summary_state(out::Dict)
    ...
    s *= "Black ppl killed by police: $(@sprintf("%d", out[:blk_kill]))" * "\n"
    s *= "Avg. killed/yr. (per mil.): $(@sprintf("%.1f", out[:avg_all_kill_py_pMM])), rank: $(@sprintf("%d", out[:avg_all_kill_py_pMM_rk]))" * "\n"
    s *= "Avg. Black ppl killed/yr. (per mil.): $(@sprintf("%.1f", out[:avg_blk_kill_py_pMM])), rank: $(@sprintf("%d", out[:avg_blk_kill_py_pMM_rk]))" * "\n"
    s *= "Black ppl killed rate = $(@sprintf("%.2fx", out[:blk_v_all_kill_py_pMM_x])) all killed rate" * "\n"
...
function display_summary_city(out::Dict)
    ...
    s *= "Avg. Black ppl killed/yr. (per mil.): $(@sprintf("%.1f", out[:avg_blk_kill_py_pMM]))" * "\n"
    s *= "Black ppl killed rate = $(@sprintf("%.2fx", out[:blk_v_all_kill_py_pMM_x])) all killed rate" * "\n"
    s *= "Black pop. proportion: $(@sprintf("%.1f", 100out[:blk_pop_pct]))%" * "\n"
    s *= "Black killed proportion: $(@sprintf("%.1f", 100out[:pct_blk_victims]))%" * "\n"
